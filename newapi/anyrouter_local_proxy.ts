...
// content-encoding/content-length: Node fetch auto-decompresses the body but
// keeps these headers describing the compressed form, so transparently
// forwarding them makes the client try to decode plaintext → ZlibError.
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // Defer res.writeHead(upstream.status, ...) until the first body byte
    // arrives. Anyrouter / new-api gateways occasionally return
    // "2xx + correct content-type + 0 bytes" when the backend silently
    // fails (quota / rate-limit / cold-start crash). The Anthropic SDK
    // does NOT retry such fake-success responses, so we rewrite them to
    // 502 below — but only possible if we haven't committed status yet.
    ...
    const commitHeaders = () => {
      if (!res.headersSent) res.writeHead(upstream.status, resHeaders);
    };
    if (upstream.body) {
      ...
      try {
        while (true) {
          ...
          if (done) break;
          // First byte received → commit upstream's status code. After this
          // point we can no longer rewrite to 502, so we delay as long as
          // possible (until we actually have data to forward).
          commitHeaders();
          totalBytes += value.byteLength;
          if (isError) errChunks.push(Buffer.from(value));
          // Always keep a sample of the first ~2KB so we can dump it on
          // suspicious 2xx (truncated body, stream cut) without buffering
          // whole multi-MB streams.
          if (sampleBytes < SAMPLE_LIMIT) {
            const need = SAMPLE_LIMIT - sampleBytes;
            sampleChunks.push(Buffer.from(value.subarray(0, need)));
            sampleBytes += Math.min(value.byteLength, need);
          }
          res.write(value);
        }
      } catch (e) {
    ...
    const tail = [
      `bytes=${totalBytes}`,
      `ct=${ct}`,
      streamErr ? `streamErr=${streamErr}` : '',
    ].filter(Boolean).join(' ');
    console.error(`[#${id}] <== ${tail}`);

    // Fake-success rewrite: 2xx + zero-byte body → 502 so the SDK's
    // exponential-backoff retry kicks in. Only safe before headers are
    // committed (which is the case iff totalBytes === 0).
    if (!isError && totalBytes === 0 && !res.headersSent) {
      console.error(`[#${id}] !! empty 2xx (status=${upstream.status} ct=${ct}) → rewriting as 502 to trigger client retry`);
      res.writeHead(502, { 'content-type': 'application/json' });
      res.end(JSON.stringify({
        error: 'Bad Gateway',
        message: `upstream returned ${upstream.status} with empty body (ct=${ct})`,
        upstream_status: upstream.status,
      }));
      return;
    }

    // Truncated / odd 2xx that wasn't fully empty — log a sample but pass
    // through, since headers are already on the wire and we can't rewrite.
    if (!isError && (totalBytes < 200 || streamErr)) {
      const sample = Buffer.concat(sampleChunks).toString('utf8');
      console.error(`[#${id}] !! suspect 2xx body(${totalBytes}B): ${JSON.stringify(sample.slice(0, 2000))}`);
    }
    // Edge case: non-2xx with empty body, or upstream.body was null —
    // headers haven't been committed by the streaming loop, do it now.
    commitHeaders();
    res.end();
  } catch (err: unknown) {
